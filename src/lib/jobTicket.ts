// ============================================================================
// Printable job ticket — pure model + HTML builder (client-safe, no deps).
//
// One ticket per production order. Operators work from this printed sheet on
// the floor — it must list every material batch / cut list, its stock
// identity, dimensions if any, and its private station-job chain with
// tick-boxes for manual sign-off. Legacy orders (no productionPlan) fall back
// to a combined Materials + shared Operations view.
// ============================================================================

export const JOB_TICKET_VERSION = 1;

function esc(value: unknown): string {
  return String(value ?? "").replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[c] as string));
}

export function jobTicketFilename(orderNumber: string): string {
  const safe = String(orderNumber || "order")
    .trim()
    .replace(/[^A-Za-z0-9._-]/g, "-")
    .replace(/-+/g, "-")
    .slice(0, 60) || "order";
  return `JobTicket-${safe}.pdf`;
}

export function formatJobDate(value: unknown): string {
  if (!value) return "—";
  const d = value instanceof Date ? value : new Date(String(value));
  if (Number.isNaN(d.getTime())) return "—";
  return d.toLocaleDateString(undefined, { year: "numeric", month: "short", day: "2-digit" });
}

export function formatJobDateTime(value: unknown): string {
  if (!value) return "—";
  const d = value instanceof Date ? value : new Date(String(value));
  if (Number.isNaN(d.getTime())) return "—";
  return d.toLocaleString(undefined, {
    year: "numeric",
    month: "short",
    day: "2-digit",
    hour: "2-digit",
    minute: "2-digit",
  });
}

function formatScheduled(start: unknown, end: unknown): string {
  if (!start || !end) return "—";
  const s = new Date(String(start));
  const e = new Date(String(end));
  if (Number.isNaN(s.getTime()) || Number.isNaN(e.getTime())) return "—";
  const sameDay = s.toDateString() === e.toDateString();
  if (sameDay) {
    return `${s.toLocaleDateString()} · ${s.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}–${e.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
  }
  return `${formatJobDateTime(s)} → ${formatJobDateTime(e)}`;
}

export interface JobTicketBuildOptions {
  inventoryItems?: any[];
  machines?: any[];
  qrDataUrl?: string | null;
  generatedAt?: string | Date;
  lang?: string;
}

/**
 * Build a self-contained printable HTML job ticket. No external assets — all
 * styles are inline and the QR is an embedded data URL when provided.
 * The HTML is designed for `window.open() + print()` and for "Save as PDF".
 */
export function buildJobTicketHtml(order: any, opts: JobTicketBuildOptions = {}): string {
  const inventoryItems: any[] = Array.isArray(opts.inventoryItems) ? opts.inventoryItems : [];
  const machines: any[] = Array.isArray(opts.machines) ? opts.machines : [];
  const qrDataUrl = opts.qrDataUrl || null;
  const generatedAt = opts.generatedAt ? new Date(String(opts.generatedAt)) : new Date();
  const genStamp = Number.isNaN(generatedAt.getTime()) ? new Date().toLocaleString() : generatedAt.toLocaleString();

  const orderNumber = String(order?.orderNumber ?? `ORDER-${order?.id ?? ""}`);
  const title = String(order?.title ?? "");
  const projectType = String(order?.projectType ?? "—");
  const priority = String(order?.priority ?? "Normal");
  const status = String(order?.status ?? "Pending");
  const dueDate = order?.dueDate ?? null;
  const createdAt = order?.createdAt ?? null;
  const notes = String(order?.notes ?? "").trim();
  const customerCompany = String(order?.customerCompany ?? order?.customerName ?? "—");
  const customerName = String(order?.customerName ?? "");
  const customerPhone = String(order?.customerPhone ?? "");
  const customerEmail = String(order?.customerEmail ?? "");
  const progress = Number(order?.progressPercent ?? 0);

  const invById = new Map<number, any>();
  for (const it of inventoryItems) {
    const id = Number(it?.id);
    if (Number.isInteger(id)) invById.set(id, it);
  }
  const machineById = new Map<number, any>();
  for (const m of machines) {
    const id = Number(m?.id);
    if (Number.isInteger(id)) machineById.set(id, m);
  }
  const opsById = new Map<number, any>();
  for (const op of (order?.operations ?? [] as any[])) {
    const id = Number(op?.id);
    if (Number.isInteger(id)) opsById.set(id, op);
  }

  const priorityTone =
    priority === "Urgent" ? "background:#fee2e2;color:#991b1b;border:1px solid #fecaca" :
    priority === "High" ? "background:#fef3c7;color:#92400e;border:1px solid #fde68a" :
    "background:#f1f5f9;color:#334155;border:1px solid #e2e8f0";

  const statusTone =
    status === "Completed" ? "background:#dcfce7;color:#166534" :
    status === "Delivered" ? "background:#ccfbf1;color:#134e4a" :
    status === "In Production" ? "background:#fef3c7;color:#92400e" :
    "background:#f1f5f9;color:#334155";

  // Batches
  const plan = order?.productionPlan ?? null;
  const hasPlan = plan && Array.isArray(plan.items) && plan.items.length > 0;

  // Helpers to render dimensions/location
  const dimFor = (itemId: number): string => {
    const it = invById.get(Number(itemId));
    if (!it) return "";
    const d = String(it.dimensions ?? "").trim();
    return d;
  };
  const locFor = (itemId: number): string => {
    const it = invById.get(Number(itemId));
    if (!it) return "";
    return String(it.location ?? "").trim();
  };
  const skuForItemId = (itemId: number): string => {
    const it = invById.get(Number(itemId));
    return it ? String(it.sku ?? "—") : "—";
  };
  const unitForItemId = (itemId: number): string => {
    const it = invById.get(Number(itemId));
    return it ? String(it.unit ?? "") : "";
  };

  // Build batches HTML
  let batchesHtml = "";
  if (hasPlan) {
    // One card per material batch with its private chain
    for (let idx = 0; idx < plan.items.length; idx++) {
      const batch: any = plan.items[idx];
      const batchNumber = idx + 1;
      const batchName = String(batch?.name ?? `Batch ${batchNumber}`);
      const itemId = Number(batch?.itemId);
      const qty = Number(batch?.quantityUsed ?? 0);
      const recipeName = String(batch?.recipeName ?? batch?.routeSource ?? "");
      const inv = invById.get(itemId);
      const sku = inv ? String(inv.sku ?? "—") : "—";
      const unit = inv ? String(inv.unit ?? "") : "";
      const dims = dimFor(itemId);
      const loc = locFor(itemId);
      const matRow = opsById.size ? null : null; // placeholder
      // Find matching material allocation for SKU fidelity (when order.materials present)
      const matAlloc = (order?.materials ?? []).find((m: any) => Number(m?.id) === Number(batch?.materialId) || Number(m?.itemId) === itemId);
      const allocSku = matAlloc?.itemSku ? String(matAlloc.itemSku) : sku;
      const allocUnit = matAlloc?.itemUnit ? String(matAlloc.itemUnit) : unit;

      const steps: any[] = Array.isArray(batch?.steps) ? batch.steps : [];
      const batchMinutes = steps.reduce((s: number, st: any) => s + (Number(st?.estimatedMinutes) || 0), 0);
      const stepRows = steps.map((st: any, sIdx: number) => {
        const opId = Number(st?.operationId);
        const live = opsById.get(opId);
        const opName = String(st?.operationName ?? live?.operationName ?? `Step ${sIdx + 1}`);
        const cat = String(st?.machineCategory ?? live?.machineCategory ?? "—");
        const mid = st?.machineId != null ? Number(st.machineId) : live?.machineId != null ? Number(live.machineId) : null;
        const machine = mid != null ? machineById.get(mid) : null;
        const station = machine ? `${esc(machine.code)} — ${esc(machine.name)}` : mid == null ? `<span style="color:#64748b">Auto-assign</span>` : `<span style="color:#64748b">Machine #${esc(String(mid))}</span>`;
        const est = Number(st?.estimatedMinutes ?? live?.estimatedMinutes ?? 0);
        const sched = live ? formatScheduled(live.scheduledStart, live.scheduledEnd) : "—";
        const liveStatus = live ? String(live.status ?? "") : "";
        const statusDot = liveStatus === "Completed" ? "✓" : liveStatus === "In Progress" ? "●" : liveStatus === "Ready" ? "○" : "·";
        return `<tr>
          <td style="text-align:center;font-weight:800">${sIdx + 1}</td>
          <td style="font-weight:700">${esc(opName)}</td>
          <td>${esc(cat)}</td>
          <td>${station}</td>
          <td style="text-align:center;font-family:monospace">${est || "—"}</td>
          <td style="font-size:10px">${esc(sched)}</td>
          <td style="text-align:center;color:#334155">${esc(statusDot)}</td>
          <td style="text-align:center"><span style="display:inline-block;width:14px;height:14px;border:1.5px solid #334155;border-radius:3px;vertical-align:middle"></span></td>
          <td style="min-width:70px;border-bottom:1px solid #94a3b8"></td>
        </tr>`;
      }).join("");

      batchesHtml += `
        <div style="border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;margin:14px 0 18px;page-break-inside:avoid">
          <div style="background:#fffbeb;border-bottom:1px solid #fde68a;padding:10px 14px;display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:8px">
            <div>
              <div style="font-size:11px;font-weight:800;letter-spacing:0.06em;color:#92400e;text-transform:uppercase">CUT LIST — BATCH ${batchNumber} · ${esc(batchName)}</div>
              <div style="font-size:11px;color:#334155;margin-top:2px">
                <span style="font-family:monospace;font-weight:800;color:#b45309">${esc(allocSku)}</span>
                <span style="margin:0 6px;color:#94a3b8">·</span>
                <span>${qty} ${esc(allocUnit || unit)}</span>
                ${dims ? `<span style="margin:0 6px;color:#94a3b8">·</span><span style="font-family:monospace;background:#f1f5f9;border:1px solid #e2e8f0;padding:1px 6px;border-radius:6px">${esc(dims)}</span>` : ""}
                ${loc ? `<span style="margin-left:6px;color:#64748b">· Loc: ${esc(loc)}</span>` : ""}
              </div>
            </div>
            <div style="text-align:right">
              <div style="font-size:10px;font-weight:800;letter-spacing:0.06em;color:#64748b;text-transform:uppercase">${recipeName ? esc(recipeName) : "Custom route"}</div>
              <div style="font-size:10px;color:#334155">${steps.length} station pass${steps.length===1?"":"es"} · ${batchMinutes} min est.</div>
            </div>
          </div>
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            <thead>
              <tr style="background:#1e293b;color:#f59e0b">
                <th style="padding:7px 6px;width:28px">#</th>
                <th style="padding:7px 8px;text-align:left">Operation</th>
                <th style="padding:7px 8px;text-align:left">Machine type</th>
                <th style="padding:7px 8px;text-align:left">Assigned station</th>
                <th style="padding:7px 6px;width:58px">Est min</th>
                <th style="padding:7px 8px;text-align:left">Scheduled slot</th>
                <th style="padding:7px 6px;width:34px">Status</th>
                <th style="padding:7px 6px;width:40px">Done ☐</th>
                <th style="padding:7px 8px;width:90px">Operator</th>
              </tr>
            </thead>
            <tbody>
              ${stepRows || `<tr><td colspan="9" style="padding:12px;text-align:center;color:#64748b">No routed passes</td></tr>`}
            </tbody>
          </table>
          <div style="padding:8px 12px;background:#f8fafc;border-top:1px solid #e2e8f0;display:flex;gap:8px;flex-wrap:wrap;font-size:10px;color:#475569">
            <span style="display:inline-flex;align-items:center;gap:4px"><span style="display:inline-block;width:10px;height:10px;border-radius:999px;background:#f59e0b"></span>Operator ticks DONE when the pass is complete</span>
            <span>·</span>
            <span>Initials = accountability</span>
          </div>
        </div>`;
    }
  } else {
    // Legacy: separate Materials and Operations tables
    const materials: any[] = Array.isArray(order?.materials) ? order.materials : [];
    const ops: any[] = Array.isArray(order?.operations) ? order.operations : [];
    const matsRows = materials.map((m: any, i: number) => {
      const itemId = Number(m?.itemId);
      const sku = String(m?.itemSku ?? skuForItemId(itemId) ?? "—");
      const name = String(m?.itemName ?? m?.productionItemName ?? invById.get(itemId)?.name ?? "Material");
      const qty = String(m?.quantityUsed ?? "—");
      const unit = String(m?.itemUnit ?? unitForItemId(itemId) ?? "");
      const dims = dimFor(itemId);
      const loc = locFor(itemId);
      return `<tr>
        <td style="text-align:center;font-weight:700">${i + 1}</td>
        <td style="font-family:monospace;font-weight:800;color:#b45309">${esc(sku)}</td>
        <td style="font-weight:700">${esc(name)}</td>
        <td style="text-align:right;font-family:monospace">${esc(qty)}</td>
        <td>${esc(unit)}</td>
        <td style="font-family:monospace">${esc(dims || "—")}</td>
        <td>${esc(loc || "—")}</td>
        <td style="text-align:center"><span style="display:inline-block;width:14px;height:14px;border:1.5px solid #334155;border-radius:3px"></span></td>
      </tr>`;
    }).join("");

    const opsRows = ops
      .slice()
      .sort((a: any, b: any) => (Number(a?.stepOrder) || 0) - (Number(b?.stepOrder) || 0))
      .map((op: any, i: number) => {
        const pos = Number(op?.stepOrder ?? i + 1);
        const opName = String(op?.operationName ?? "");
        const cat = String(op?.machineCategory ?? "—");
        const mid = op?.machineId != null ? Number(op.machineId) : null;
        const machine = mid != null ? machineById.get(mid) : null;
        const station = machine ? `${esc(machine.code)} — ${esc(machine.name)}` : mid == null ? `<span style="color:#64748b">Auto</span>` : `Machine #${esc(String(mid))}`;
        const est = Number(op?.estimatedMinutes ?? 0);
        const sched = formatScheduled(op?.scheduledStart, op?.scheduledEnd);
        const st = String(op?.status ?? "");
        const dot = st === "Completed" ? "✓" : st === "In Progress" ? "●" : st === "Ready" ? "○" : "·";
        return `<tr>
          <td style="text-align:center;font-weight:800">${pos}</td>
          <td style="font-weight:700">${esc(opName)}</td>
          <td>${esc(cat)}</td>
          <td>${station}</td>
          <td style="text-align:center;font-family:monospace">${est || "—"}</td>
          <td style="font-size:10px">${esc(sched)}</td>
          <td style="text-align:center">${esc(dot)}</td>
          <td style="text-align:center"><span style="display:inline-block;width:14px;height:14px;border:1.5px solid #334155;border-radius:3px"></span></td>
          <td style="min-width:80px;border-bottom:1px solid #94a3b8"></td>
        </tr>`;
      }).join("");

    batchesHtml = `
      <div style="margin:14px 0">
        <div style="font-size:11px;font-weight:800;letter-spacing:0.06em;color:#334155;text-transform:uppercase;margin-bottom:6px">Materials / Cut list</div>
        <table style="width:100%;border-collapse:collapse;font-size:11px">
          <thead><tr style="background:#1e293b;color:#f59e0b">
            <th style="padding:7px 6px;width:28px">#</th>
            <th style="padding:7px 8px;text-align:left">SKU</th>
            <th style="padding:7px 8px;text-align:left">Item</th>
            <th style="padding:7px 6px;width:58px">Qty</th>
            <th style="padding:7px 6px;width:46px">Unit</th>
            <th style="padding:7px 8px;text-align:left">Dimensions</th>
            <th style="padding:7px 8px;text-align:left">Location</th>
            <th style="padding:7px 6px;width:50px">Picked ☐</th>
          </tr></thead>
          <tbody>${matsRows || `<tr><td colspan="8" style="padding:12px;text-align:center;color:#94a3b8">No materials allocated</td></tr>`}</tbody>
        </table>
      </div>
      <div style="margin:18px 0 8px">
        <div style="font-size:11px;font-weight:800;letter-spacing:0.06em;color:#334155;text-transform:uppercase;margin-bottom:6px">Production routing — sequential passes</div>
        <table style="width:100%;border-collapse:collapse;font-size:11px">
          <thead><tr style="background:#1e293b;color:#f59e0b">
            <th style="padding:7px 6px;width:28px">#</th>
            <th style="padding:7px 8px;text-align:left">Operation</th>
            <th style="padding:7px 8px;text-align:left">Machine type</th>
            <th style="padding:7px 8px;text-align:left">Station</th>
            <th style="padding:7px 6px;width:58px">Est min</th>
            <th style="padding:7px 8px;text-align:left">Scheduled slot</th>
            <th style="padding:7px 6px;width:34px">Status</th>
            <th style="padding:7px 6px;width:40px">Done ☐</th>
            <th style="padding:7px 8px;width:90px">Operator</th>
          </tr></thead>
          <tbody>${opsRows || `<tr><td colspan="9" style="padding:12px;text-align:center;color:#94a3b8">No routed passes</td></tr>`}</tbody>
        </table>
      </div>`;
  }

  const totalMats = hasPlan ? plan.items.length : (order?.materials ?? []).length;
  const totalOps = hasPlan
    ? plan.items.reduce((s: number, it: any) => s + (Array.isArray(it?.steps) ? it.steps.length : 0), 0)
    : (order?.operations ?? []).length;
  const totalMinutes = hasPlan
    ? plan.items.reduce((s: number, it: any) => s + (Array.isArray(it?.steps) ? it.steps.reduce((a: number, st: any) => a + (Number(st?.estimatedMinutes) || 0), 0) : 0), 0)
    : (order?.operations ?? []).reduce((s: number, op: any) => s + (Number(op?.estimatedMinutes) || 0), 0);

  const dueTone = (() => {
    if (!dueDate) return "";
    const d = new Date(String(dueDate));
    if (Number.isNaN(d.getTime())) return "";
    const now = new Date();
    const diff = d.getTime() - now.getTime();
    if (diff < 0) return "color:#dc2626;font-weight:800";
    if (diff < 3 * 24 * 3600 * 1000) return "color:#d97706;font-weight:800";
    return "";
  })();

  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>Job Ticket ${esc(orderNumber)}</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<style>
  @page { size: A4; margin: 10mm; }
  * { box-sizing: border-box; }
  body { font-family: "Inter", "Segoe UI", Arial, Helvetica, sans-serif; color:#0f172a; margin:0; padding:0; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  @media print { .no-print { display:none !important; } }
  a { color: inherit; }
  .cut { border-top: 1px dashed #94a3b8; margin: 10px 0; position:relative; }
  .cut span { position:absolute; top:-7px; left:50%; transform:translateX(-50%); background:#fff; padding:0 8px; font-size:9px; letter-spacing:0.1em; color:#94a3b8; }
</style>
</head>
<body>
  <div style="max-width:800px;margin:0 auto;padding:18px 18px 24px">

    <!-- Brand band -->
    <div style="background:#0f172a;border-radius:14px;overflow:hidden;border:1px solid #1e293b">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;padding:16px 18px 14px">
        <div>
          <div style="color:#f59e0b;font-weight:900;letter-spacing:0.14em;font-size:18px;line-height:1">WOODTEK</div>
          <div style="color:#cbd5e1;font-size:10px;letter-spacing:0.12em;text-transform:uppercase;margin-top:3px">Furniture Service Center</div>
          <div style="margin-top:10px;display:inline-flex;align-items:center;gap:8px;background:#f59e0b;color:#0f172a;font-weight:900;font-size:11px;letter-spacing:0.1em;padding:5px 10px;border-radius:999px;text-transform:uppercase">Job Ticket</div>
        </div>
        <div style="text-align:right;min-width:180px">
          <div style="font-family:monospace;font-weight:900;font-size:22px;color:#f59e0b;letter-spacing:0.02em;line-height:1">${esc(orderNumber)}</div>
          <div style="color:#e2e8f0;font-size:11px;font-weight:700;margin-top:3px;max-width:320px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(title || "—")}</div>
          <div style="margin-top:6px;display:flex;justify-content:flex-end;gap:6px;flex-wrap:wrap">
            <span style="font-size:10px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;padding:4px 8px;border-radius:999px;${statusTone}">${esc(status)}</span>
            <span style="font-size:10px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;padding:4px 8px;border-radius:999px;${priorityTone}">${esc(priority)}</span>
          </div>
        </div>
      </div>
      <div style="height:4px;background:#f59e0b"></div>
      <div style="display:flex;justify-content:space-between;gap:12px;padding:10px 18px;background:#1e293b;color:#cbd5e1;font-size:10px">
        <span>Issued: ${esc(formatJobDate(createdAt))} · Due: <span style="${dueTone}">${esc(formatJobDate(dueDate))}</span> · Progress: ${esc(String(progress))}%</span>
        <span>Printed: ${esc(genStamp)}</span>
      </div>
    </div>

    <!-- Order + client meta -->
    <div style="display:grid;grid-template-columns:1.15fr 0.85fr;gap:12px;margin-top:14px">
      <div style="border:1px solid #e2e8f0;border-radius:12px;padding:12px 14px;background:#fff">
        <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#64748b;text-transform:uppercase">Client</div>
        <div style="font-size:13px;font-weight:900;color:#0f172a;margin-top:4px">${esc(customerCompany)}</div>
        ${customerName ? `<div style="font-size:11px;color:#334155;margin-top:2px">Attn: ${esc(customerName)}</div>` : ""}
        ${customerPhone ? `<div style="font-size:11px;color:#334155">Tel: ${esc(customerPhone)}</div>` : ""}
        ${customerEmail ? `<div style="font-size:11px;color:#334155">Email: ${esc(customerEmail)}</div>` : ""}
        <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;font-size:10px">
          <span style="background:#f1f5f9;border:1px solid #e2e8f0;padding:3px 8px;border-radius:999px">Type: <strong>${esc(projectType)}</strong></span>
          ${orderNumber ? `<span style="background:#fffbeb;border:1px solid #fde68a;color:#92400e;padding:3px 8px;border-radius:999px">Ref: <strong style="font-family:monospace">${esc(orderNumber)}</strong></span>` : ""}
        </div>
      </div>
      <div style="border:1px solid #e2e8f0;border-radius:12px;padding:12px 14px;background:#f8fafc;display:flex;gap:12px;align-items:center">
        <div style="flex:1">
          <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#64748b;text-transform:uppercase">Scan to open</div>
          <div style="font-size:10px;color:#475569;margin-top:4px;line-height:1.4">Scan on any shop device to jump straight to this order's workflow.</div>
          <div style="font-size:10px;color:#64748b;margin-top:6px;font-family:monospace;word-break:break-all">${esc(orderNumber)} · ${esc(String(order?.id ?? ""))}</div>
        </div>
        ${qrDataUrl ? `<img src="${qrDataUrl}" alt="Order QR" style="width:92px;height:92px;border-radius:10px;border:1px solid #e2e8f0;background:#fff;padding:6px">` : `<div style="width:92px;height:92px;border-radius:10px;border:1px dashed #cbd5e1;background:#fff;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:10px;text-align:center;padding:8px">QR<br>available<br>on screen</div>`}
      </div>
    </div>

    <!-- Production summary chips -->
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px">
      <span style="background:#0f172a;color:#f59e0b;font-weight:800;font-size:11px;padding:6px 10px;border-radius:999px">${totalMats} material batch${totalMats===1?"":"es"}</span>
      <span style="background:#f1f5f9;border:1px solid #e2e8f0;color:#334155;font-weight:800;font-size:11px;padding:6px 10px;border-radius:999px">${totalOps} station pass${totalOps===1?"":"es"} · ${Math.floor(totalMinutes/60)}h ${totalMinutes%60}m est.</span>
      ${progress ? `<span style="background:${progress===100?"#dcfce7":"#fef3c7"};color:${progress===100?"#166534":"#92400e"};font-weight:800;font-size:11px;padding:6px 10px;border-radius:999px">Progress ${esc(String(progress))}%</span>` : ""}
    </div>

    <!-- Batches -->
    ${batchesHtml}

    <!-- Notes -->
    ${notes ? `<div style="margin-top:14px;border:1px solid #fde68a;background:#fffbeb;border-radius:12px;padding:12px 14px">
      <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#92400e;text-transform:uppercase">Client / engineering notes</div>
      <div style="font-size:11px;color:#334155;margin-top:6px;white-space:pre-wrap;line-height:1.5">${esc(notes)}</div>
    </div>` : ""}

    <!-- Floor instructions -->
    <div style="margin-top:14px;border:1px solid #e2e8f0;border-radius:12px;padding:12px 14px;background:#f8fafc">
      <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#334155;text-transform:uppercase">Floor instructions</div>
      <ul style="margin:6px 0 0 16px;padding:0;font-size:11px;color:#334155;line-height:1.6">
        <li>Work <strong>one batch at a time</strong>. Each batch is a separate cut list with its own routing — do not mix pieces across batches.</li>
        <li>Check material <strong>dimensions</strong> and <strong>quantity</strong> before starting a batch. Report shortages immediately.</li>
        <li>Tick <strong>Done ☐</strong> and sign <strong>Operator</strong> after each station pass. The next station starts only after you tick.</li>
        <li>If a piece fails QC, keep it with the ticket and mark <strong>Rework</strong> — do not advance the batch.</li>
      </ul>
    </div>

    <!-- Signatures -->
    <div style="margin-top:18px;display:grid;grid-template-columns:1fr 1fr;gap:18px">
      <div>
        <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#64748b;text-transform:uppercase">Operator signature</div>
        <div style="margin-top:18px;border-bottom:1.5px solid #0f172a;height:18px"></div>
        <div style="font-size:9px;color:#64748b;margin-top:4px">Name / signature · Date & time</div>
      </div>
      <div>
        <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#64748b;text-transform:uppercase">Supervisor / QA sign-off</div>
        <div style="margin-top:18px;border-bottom:1.5px solid #0f172a;height:18px"></div>
        <div style="font-size:9px;color:#64748b;margin-top:4px">Name / signature · Date & time</div>
      </div>
    </div>
    <div style="margin-top:18px;display:grid;grid-template-columns:1fr 1fr;gap:18px">
      <div>
        <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#64748b;text-transform:uppercase">QC checked</div>
        <div style="margin-top:18px;border-bottom:1.5px solid #0f172a;height:18px"></div>
        <div style="font-size:9px;color:#64748b;margin-top:4px">QC inspector · Date & time</div>
      </div>
      <div>
        <div style="font-size:10px;font-weight:800;letter-spacing:0.08em;color:#64748b;text-transform:uppercase">Delivery / dispatch</div>
        <div style="margin-top:18px;border-bottom:1.5px solid #0f172a;height:18px"></div>
        <div style="font-size:9px;color:#64748b;margin-top:4px">Packed by · Date & time</div>
      </div>
    </div>

    <div style="margin-top:18px;text-align:center;font-size:9px;color:#94a3b8">
      WoodTek ERP — Furniture Service Center · Job ticket travels with the job · Generated ${esc(genStamp)} · ${esc(orderNumber)}
    </div>

    <div class="cut no-print" style="margin-top:16px"><span>✂ cut here — keep with the batch</span></div>

    <div class="no-print" style="margin-top:16px;display:flex;justify-content:center;gap:8px">
      <button onclick="window.print()" style="background:#0f172a;color:#f59e0b;border:1px solid #1e293b;font-weight:800;font-size:12px;padding:10px 18px;border-radius:10px;cursor:pointer">🖨 Print ticket</button>
      <button onclick="window.close()" style="background:#fff;border:1px solid #e2e8f0;color:#334155;font-weight:700;font-size:12px;padding:10px 18px;border-radius:10px;cursor:pointer">Close</button>
    </div>
  </div>
</body>
</html>`;
}
